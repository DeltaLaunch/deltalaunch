...
void Cache::draw(SDL_Renderer* renderer, const SDL_Rect& rect, const std::string& s, u8 fontIndex) {
    ...
    settings.x = 0;
    settings.y = 0;
    settings.r = rect; // SDL_Rect{ 0,0,rect.w, rect.h };
    settings.color = m_color;
...
void Cache::drawWord(SDL_Renderer* renderer, Context& context, char*& ptr, const char* end, u8 fontIndex) {
...
u64 Cache::sizeWord(SDL_Renderer* renderer, Context& context, char* ptr, const char* end) {
    u64 width = 0;
    while (!isSpace(*ptr) && ptr < end) {
        u32 ordinal = utf8char(ptr, end);
        Glyph& g = m_glyphs[ordinal];
...
u64 Cache::drawParagraph(SDL_Renderer* renderer, const SDL_Rect& r, const std::string& s, u8 fontIndex) {
    ...
    while (ptr < end) {
        ...
        if (ptr < end) {
            u64 width = sizeWord(renderer, settings, ptr, end);
            ...
            drawWord(renderer, settings, ptr, end, fontIndex);
        }
    }
    return settings.y;
}
...
int Cache::getFontIndex(u32 c, u8 fontIndex) {
    ...
    if (c >= 0xEE8080 && c <= 0xEE8592)
        return Extended;

    if (c >= 0xE18480 && c <= 0xE187BF)
        return Korean;

    if (c >= 0xEAB080 && c <= 0xED9EA3)
        return Korean;

    if (c >= 0xE384B0 && c <= 0xE3868F)
        return Korean;

    if (c >= 0xEAA5A0 && c <= 0xEAA5BF)
        return Korean;

    if (c >= 0xED9EB0 && c <= 0xED9FBF)
        return Korean;


    if (c >= 0xE4B880 && c <= 0xE9BFBF)
        return Chinese_Simplified;

    if (c >= 0xEE8880 && c <= 0xEE8C80)
        return SYMBOLS1;
